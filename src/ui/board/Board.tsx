...
export function Board({ board }: { board: string[][] }) {
    if (typeof window === "undefined") {
        return null; // Возвращаем null на сервере, чтобы избежать ошибок с хуками
    }
    //Если `board` вдруг `null`, используем `initialState`
    const [state, dispatch] = useReducer(gameReducer, { 
        ...initialState, 
        //board: board || initialState.board 
        board: board?.length ? board : initialState.board
    });
                ...
                {row.map((value, x) => (
                     <Box
                        ...
                        sx={{
                            ...
                            onClick={() => value === "" && dispatch({ type: "MAKE_MOVE", x, y })}
                    >
                        {value || ""}
                    </Box>
                ))}
